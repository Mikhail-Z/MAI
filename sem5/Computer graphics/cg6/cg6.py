# программа для создания анимационного эффекта (изменение прозрачности точек) при приближении фигуры (8 вариант).
    ...
    for surface in surfaces:
        if len(surface) == 4:
            ...
            for i in surface:
                glVertex3f(verticies[i][0], verticies[i][1], verticies[i][2])
            glEnd()
        # разрешить наложение цветов
        glEnable(GL_BLEND)
        # алгоритм, по которуму будут смешиваться два цвета
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
        glPointSize(5.0)
        glBegin(GL_POINTS)
        glColor4f(1, 1, 0, alpha)
        for point in surface:
            glVertex3f(verticies[point][0], verticies[point][1], verticies[point][2])
        glEnd()
        glDisable(GL_BLEND)
        k += 1
...
class Application(tk.Tk):
    def __init__(self, *args, **kwargs):  # конструктор класса
        ...
        self.btn.pack()
        tk.Label(self, text="press +/- for zooming").pack()

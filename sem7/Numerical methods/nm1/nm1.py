...
def impl_fin_dif_scheme_appr2(u, tay, h, m, n, par_a):
    ...
    for k in range(m - 1):
        ...
        for j in range(1, n - 1):
            ...
            c[j] = sigma
            d[j] = -u[k][j] - tay * additional_member(time, j * h)
        a[n - 1] = a[n - 2] * ((-3 / (2 * h)) / c[n - 2]) + 1 / (2 * h)
        b[n - 1] = b[n - 2] * ((-3 / (2 * h)) / c[n - 2]) - 4 / (2 * h)
        c[n - 1] = c[n - 2] * ((-3 / (2 * h)) / c[n - 2]) + 3 / (2 * h)
        d[n - 1] = d[n - 2] * ((-3 / (2 * h)) / c[n - 2]) + term2(time)
        print(a[n - 1], b[n - 1])
        Y = solve(a, b, c, d, n)
        print("Условие устойчивости")
        for i in range(n):
...
def impl_fin_dif_scheme_appr3(u, tay, h, m, n, par_a):
    ...
    for k in range(m - 1):
        ...
        for j in range(1, n - 1):
            ...
            c[j] = sigma / 2
            d[j] = -(
            sigma / 2 * u[k][j - 1] + (1 - sigma) * u[k][j] + sigma / 2 * u[k][j + 1]) - tay * additional_member(time,
                                                                                                                 j * h)
        a[n - 1] = -2 / h
        b[n - 1] = 2 / h + h / tay
        c[n - 1] = 0
